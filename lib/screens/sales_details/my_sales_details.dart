...
import 'package:tiendita/providers/sales_providers.dart';
...
class _SalesDetailsState extends ConsumerState<SalesDetails>{
  ...
  Future<void> _exportSalesPdf(List<dynamic> sales, double total) async {
    try{
    final kPdf = pw.Document();
    final tableData = sales.map((sale) {
      return [
        sale.numSales ?? "N/A",
        sale.salesDate != null ?
        DateFormat("dd/MM/yyyy hh:mm a").format(sale.salesDate!) : 'Sin fecha',
        '\$${sale.total.toStringAsFixed(2) ?? '.00'}',
      ];
    }).toList();
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Text(
              'Ingresos Totales: \$${total.toStringAsFixed(2)}',
              style: pw.TextStyle(fontSize: 16, color: PdfColors.deepOrange),
            ),
            pw.SizedBox(height: 20),
            pw.TableHelper.fromTextArray(
                headers: ['Ticket de venta', 'Fecha y Hora', 'Total'],
                data: tableData,
    ...
    await Printing.layoutPdf(
      ...
      name: 'Reporte_Ventas_$periodName.pdf',
    );
    }catch(e){
  ...
  Future<void> _exportSalesExcel(List<dynamic> sales, double total) async{
    try{

      String sheetName = 'Reporte_Ventas_${getDay(DateFormat('EEEE').format(now))}_${getMonth(DateFormat.MMMM().format(now))}_${now.year}}';
      ...
      sales.map((sale){
        return [
          sheetObj.appendRow([
            TextCellValue(sale.numSales ?? "N/A"),
            TextCellValue(sale.salesDate != null ? DateFormat("dd/MM/yyyy hh:mm a").format(sale.salesDate!) : 'Sin fecha'),
            TextCellValue('\$${sale.total.toStringAsFixed(2) ?? '.00'}'),
          ]),
        ];
      }).toList();
      ...
      if(kFileBytes != null){
        ...
        final filePath = '${directory.path}/Reporte_Ventas_${getDay(DateFormat('EEEE').format(now))}_${getMonth(DateFormat.MMMM().format(now))}_${now.year}}.xlsx';
                                  ...
                                  children: [
                                    Expanded(
                                      child: ElevatedButton.icon(
                                        onPressed: () => _exportSalesPdf(sales, totalEarnings),
                                    ...
                                    Expanded(
                                      child: ElevatedButton.icon(
                                          onPressed: () => _exportSalesExcel(sales, totalEarnings),
                                          label: const Text('Exportart a Excel', style: TextStyle(color: Colors.white)),
