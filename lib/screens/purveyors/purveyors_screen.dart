      ...
      if (kFileBytes != null) {
        ...
        if (mounted) {
          await SharePlus.instance.share(
            ShareParams(files: [XFile(filePath)], text: 'Lista de Proveedores'),
          );
        }
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            Icon(Icons.contact_page, color: Colors.white, size: 20),
        ...
        children: <Widget>[
          ...
          Expanded(
            child: purveyorListAsync.when(
              data: (purveyors) {
                final filteredPurveyors = purveyors
                    .whereType<PurveyorsModel?>()
                    .where((purveyor) {
                      final purveyorName =
                          purveyor?.PurveyorName.toLowerCase() ?? "";
          ...
          children: [
            FooterButton("Exportar a Excel", "images/excel.png", () {
              ...
              if (currentData != null && currentData.isNotEmpty) {
                final filteredPurveyors = currentData
                    .whereType<PurveyorsModel>()
                    .where((pur) {
                      return pur.PurveyorName.toLowerCase().contains(
                        _searchQuery,
                      );
                    })
                    .toList();
